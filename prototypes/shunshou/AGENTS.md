...
The user selected the black/yellow "顺手" setup mock on 2026-09-08. Preserve the numbered three-step layout, masked access code, ruled preference rows, and split yellow installation button. Do not use green. This isolated prototype uses demo-only access codes and explicitly simulated installation/connection results; do not connect production services or incur costs without a separate request. App-specific dark chrome styling lives in prototype.css; protected runtime source files remain unchanged.
