...
const license = readFileSync(new URL('../prototypes/shunshou/node_modules/@radix-ui/react-icons/LICENSE', import.meta.url), 'utf8');
const selected = {
...
writeFileSync(new URL('icons.svg', dir), `<!-- ${license} -->\n<svg xmlns="http://www.w3.org/2000/svg">${symbols}</svg>`);
const arrow = renderToStaticMarkup(React.createElement(icons.ArrowTopRightIcon, { width: 44, height: 44, color: '#ffcc29' }));
writeFileSync(new URL('favicon.svg', dir), `<!-- ${license} -->\n<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" rx="8" fill="#101010"/><g transform="translate(10 10)">${arrow}</g></svg>`);
